...
In the previous post, we looked at the general design of the DB, and how to traverse its trees.
...
Tuples are not really an option: you need to know the types you’re accessing at compile time, which defeats the purpose of this library.
...
```zig
const TypeEnum = EnumFromType(node_types);
```
...
But to return a pointer to a tagged union, you must actually _have_ a union stored in memory. This would require that my data by stored as unions in a single array that contained all my types. That’s not my case: I have one backing array per type.
